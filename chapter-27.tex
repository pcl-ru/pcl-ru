...
таблица -- список, сохраненный в переменной \code{*db*}.  Другой проблемой является то,
...
Сейчас вы будете решать обе проблемы путем определения класса \code{table}, который будет
...
простой оберткой для функции \code{MAKE-ARRAY}, которая создает пустой вектор с изменяемым
...
Раздел \code{:use} дает возможность доступа к функциям и макросам, чьи имена
...
\code{value-normalizer}.  Вы передаете объект типа \code{column} поскольку для функции
...
\code{LIST} и \code{MAKE-INSTANCE}.  Но вы скоро заметите, что вы часто создаете множество
...
Теперь вы можете определять методы данной обобщенной функции, специализированные для типа,
...
подкласс \code{column} -- \code{interned-values-column}, который добавит еще один слот,
...
С помощью данных методов, определенных для \code{make-column}, вы теперь можете определить
функцию, \code{make-schema}, которая создат список объектов типа \code{column} из списка
...
\code{insert-row}, которая получает список свойств (plist) имен и значений, и таблицу, и
добавляет строку к таблице.  Большая часть работы выполняется в дополнительной функции
\code{normalize-row}, которая создает список свойств для всех колонок таблицы, используя
...
\code{insert-row}.  Однако, вы должны сконвертировать строки, возвращенные функциями
...
библиотеки переносимых имен файлов, а также функцией \code{mp3-p} из библиотеки
...
\code{:order-by} (если он указан), должен быть списком имен колонок; результаты будут
...
значение, возвращенное функцией \code{extractor} в качестве аргумента функции \code{MAP}.
Поскольку \code{project-columns} предназначена для возврата набора строк с только
определенными значениями колонок, вы можете заключить, что \code{extractor} возвращает
...
когда пара значений не проходит тест, в противном случае \code{LOOP} вернет \code{T}.
...
вернет функцию, которая будет выбирать строки, в которых значение колонки \code{:artist}
равно \code{"Green Day"}.  Вы также можете передавать множество имен колонок и значений --
...
Затем вы создаете список функций \code{column-matching} для заданных имен и значений,
...
передаете еще одно замыкание, которое применит \code{FUNCALL} к функции отбора колонки,
...
обеспечивает базовый способ организации циклов, и функцию \code{map-rows}, которая создает
